...
2. Stage relevant files by name — never `git add -A` or `git add .` (avoid pulling in `.env`, credentials, large binaries, or personal library data like `library.json`/`goodreads_import.csv`/`test_import.csv`).
