# Self-contained setup for Audible -> Goodreads.
# Installs Python if it isn't already present (per-user, no admin rights
# required), installs the project's dependencies, then launches the app.
...
Write-Host "Audible to Goodreads"
Write-Host "===================="
Write-Host ""
...
& $python -m pip show audible-cli *> $null
if ($LASTEXITCODE -ne 0) {
    Write-Host "Installing audible-cli..."
    & $python -m pip install audible-cli --quiet --disable-pip-version-check
}

Write-Host ""
Write-Host "Starting... a browser window will open automatically."
Write-Host "Keep this window open while you use the app."
Write-Host ""

& $python (Join-Path $RepoDir "app.py")
exit $LASTEXITCODE
